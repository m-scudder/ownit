...
export const Button: React.FC<{ label: string; onPress: () => void; tone?: 'default' | 'danger' | 'ghost'; disabled?: boolean; style?: ViewStyle }> = ({ label, onPress, tone = 'default', disabled, style }) => (
  <TouchableOpacity onPress={onPress} disabled={disabled} style={[styles.button, tone === 'danger' && { backgroundColor: colors.border }, tone === 'ghost' && { backgroundColor: 'transparent', borderColor: colors.border, borderWidth: 1 }, disabled && { opacity: 0.6 }, style]}>
...
export const Row: React.FC<{ children: React.ReactNode; style?: ViewStyle }> = ({ children, style }) => <View style={[styles.row, style]}>{children}</View>;
